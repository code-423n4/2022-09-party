...
There are some proposals that can take multiple executions to complete (eg. `ListOnZoraProposal`), as opposed to ones that are completed in one like it commonly works in other governance implementations. While a multi-step proposal is in progress, no other proposals can be executed until it is finished. There is support for canceling proposals should they get "stuck" for any reason.
...
- By controlling the `Globals` registry contract, it’s understood that the PartyDAO multisig has a lot of power over the protocol. For example, the multisig can simply replace the canonical `TokenDistributor` contract for all future distributions at any time. Also, certain proposal parameter bounds are looked up in the `Globals` registry during execution, which could limit the actions a governance party can take. On top of all this, there are emergency admin functions on governance parties and the token distributor callable by the multisig that can execute arbitrary bytecode, though party hosts can disable them on governance parties.
